...
import it.octogram.android.utils.ai.linkit.LinkitAiHelper;
...
public class MainAiHelper {
    ...
    public static int getEnabledProvidersCount() {
        ...
        if (LinkitAiHelper.isAvailable()) i++;
        return i;
    }
    ...
    public static AiProvidersDetails getPreferredProvider() {
        AiProvidersDetails favoriteProvider = GeminiHelper.isAvailable() ? AiProvidersDetails.GEMINI : ChatGPTHelper.isAvailable() ? AiProvidersDetails.CHATGPT : OpenRouterHelper.isAvailable() ? AiProvidersDetails.OPENROUTER : AiProvidersDetails.LINKIT;
        ...
        } else if (recentProvider == AiProvidersDetails.LINKIT.getId() && LinkitAiHelper.isAvailable()) {
            favoriteProvider = AiProvidersDetails.LINKIT;
        }
    ...
    public static boolean isProviderAvailable(AiProvidersDetails preferredProvider) {
        ...
        } else if (preferredProvider == AiProvidersDetails.LINKIT) {
            return LinkitAiHelper.isAvailable();
        } else {
    ...
    private static void request(AiPrompt aiPrompt, AiProvidersDetails preferredProvider, OnResultState state, boolean forced) {
        ...
        if (preferredProvider == AiProvidersDetails.GEMINI) {
            ...
            LinkitAiHelper.prompt(aiPrompt, state);
        } else {
            OctoLogging.e(TAG, "Unknown provider: " + preferredProvider);
            state.onFailed();
        }
